...
101       continue
  ...
  SUBROUTINE fill_plintab(iz,cosm,CAMB_PK)
    ...
    DO i=1,nk
       ...
       !WRITE(*,*) i, nk, cosm%k_plin(i), cosm%plin(i), cosm%plinc(i)
       !WRITE(7,*) i, nk, cosm%k_plin(i), cosm%plin(i), cosm%plinc(i)
    END DO
  ...
  FUNCTION neff(lut,cosm)
    ...
    neff=-3.-derivative_table(log(lut%rnl),log(lut%rr),log(lut%sig**2.),SIZE(lut%rr),3,3)
