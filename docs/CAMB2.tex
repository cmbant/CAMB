...
$\sim3\times10^{-2}$. We have checked that the cap is harmless at the observable
level and that $\beta_\star=30$ is not finely tuned: for dark-energy models from
near-$\Lambda$CDM through phantom-crossing to non-flat crossing cases, the default
capped code reproduces the un-regularized \cref{eq:ppf-gamma} integrated at a much
tighter tolerance to $\lesssim10^{-5}$ in the lensed $C_\ell$, matter power and
lensing potential---well below the accuracy target---with $\beta_\star$ anywhere in
$10$--$300$ giving the same converged result.\footnote{Using the PPF fluid with
$c_s^2=1$: $w_0=-0.98$; $(w_0,w_a)=(-0.9,\,0.3)$ (thawing, no crossing);
$(-0.9,\,-0.5)$ (crossing); $(-1.15,\,0.4)$ (strongly phantom); and
$\Omega_K=-0.03$ with $(-0.9,\,-0.5)$. Against the un-regularized reference
(\cref{eq:ppf-gamma} with the perturbation tolerance tightened by
$e^5\approx150$), the default $\beta_\star=30$ code agrees to $<6\times10^{-6}$
($TT$), $<2\times10^{-6}$ ($EE$), $<10^{-6}$ (linear $P(k)$,
$k\le2\,h\,\mathrm{Mpc}^{-1}$) and $<10^{-6}$ ($C_\ell^{\phi\phi}$), at the
run-to-run numerical floor; varying $\beta_\star\in\{10,30,100,300\}$ changes these
by $\lesssim4\times10^{-5}$.} Integrating the un-regularized equation at the
default tolerance instead mis-estimates the lensed $TT$ spectrum by
$\sim2\times10^{-3}$ and is strongly tolerance-dependent, so the cap removes this
sensitivity---by more than two orders of magnitude---as well as the tiny
integration steps the stiff equation would otherwise force. This is the default
PPF behaviour.
