...
containing CMB spectra, we evaluate the Gaussian $\Delta\chi^2$ between the
default and boosted-reference runs over $2\le\ell\le\min(4000,\,\ell_{\max}-200)$,
excluding the documented endpoint region near the requested $\ell_{\max}$.
...
using low lensing source cutoff $\max(k\eta_0)=10375$ tuned for
...
agents (Claude Code with Opus 4.8 or Fable, Codex with GPT 5.5/5.6 and Copilot with GPT 5.4/Claude Sonnet) under my supervision, with some cross-checks using Gemini 3.1 Pro; I am responsible
