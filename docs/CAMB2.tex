...
evolution, and CMB lensing accuracy improvements, and discuss the different possible definitions of the linear matter power spectrum in the presence of strongly non-linear sources from reionization. Accuracy is assessed
...
the expensive fixed precision of general-purpose libraries.
\end{abstract}
...
it is straightforward to fudge \textsc{HyRec}'s helium equation to match, giving accurate agreement
...
overestimates because the rescattered signal originates at lower redshift than
the primary CMB.} When accurate $BB$ is required -- for tensor $B$-mode analysis for
...
pointwise relative comparisons where the spectra cross zero. The default,
...
by a script distributed with the code (in \code{fortran/tests}), so the exact
validation set is reproducible from the source distribution rather than a
...
defaults described here are designed to reach that accuracy without boosting.
...
\textsc{CLASS} default, native reach, \(\ell=4000\) & 10 & 3.4 \\
\textsc{CLASS} \code{cl_permille}, native reach, \(\ell=4000\) & 11 & 3.7 \\
\textsc{CLASS} default, high-\(k\), \(\ell=6000\to4000\) & 15 & 4.6 \\
\textsc{CLASS} \code{cl_permille}, high-\(k\), \(\ell=6000\to4000\) & 17 & 5.1 \\
\hline
...
$5.30\,$s at $\ell_{\max}=4000$, a factor of $17$ at higher target accuracy, and
...
wide region of hyperspherical-Bessel parameter/argument space. Small $\nu/l$ and other domains included here are rarely encountered in the CMB calculation, but the \code{phi_olver} function is tested to be valid over the full range of its arguments for completeness.
...
\code{IntTolBoost} together, which empirically brackets the converged result.
\Cref{tab:tol} lists
