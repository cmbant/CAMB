...
evolution, CMB lensing accuracy work, and discuss possible different definitions of the linear matter power spectrum in the presence of strongly non-linear sources from reionization. Accuracy is assessed
with a deterministic validation harness that compares default runs to
boosted references. Essentially all of the new code and algorithms were co-developed with AI agents under human supervision. The non-flat implementation is an example of building \emph{custom-precision} numerical libraries --
numerical implementations AI-generated and calibrated to exactly the accuracy a problem need instead of wasting compute on the unnecessary precision of standard libraries.
\end{abstract}
...
Refs.~\cite{Kosowsky:1998nc}, we map it onto the
...
post-Friedmann (PPF) dark-energy evolution (\cref{sec:de}), and the curvature and
lensing accuracy work (\cref{sec:lensing,sec:curvature}).

The second contribution is methodological. Almost all of this work was
co-developed with AI agents, with a human in the loop, driven by an objective
accuracy harness (\cref{sec:validation,sec:agentic}). This is a concrete example
of a shift that we expect to become widespread: away from general-purpose
special-function libraries with a fixed accuracy/speed trade-off, and towards
\emph{custom-precision} numerical methods, hand-fitted and exhaustively
calibrated to exactly the accuracy a given problem needs on the grid it actually
samples. The hyperspherical Bessel routine is the clearest example, but the same
philosophy -- spend accuracy where it matters, and prove it with a reproducible
test -- runs through all of the changes.
...
CMB and matter-power accuracy (\cref{sec:lensing}), and curvature-specific
checks (\cref{sec:curvature}). We then describe the validation methodology
...
As a quantitative CMB-impact diagnostic, the accuracy checking code use a simple Gaussian
...
can cause instabilities in the apodized trucated correlation function calculation.
...
overestimtes due to the lower source redshift than the primary CMB.}. When accurate $BB$ is required -- for tensor B-mode analysis for
...
feedback on the small-scale matter power can be included with the \textsc{SP(k)}
model~\cite{Salcido:2023etq}.

\section{Curvature}
\label{sec:curvature}

The near-flat shortcuts of \cref{sec:bessel} are designed so that the curved
calculation varies continuously into the flat one as $\Omega_K\to0$. We verify
this explicitly by sweeping $\Omega_K$ through zero at fixed cosmology and
checking that the lensed $C_\ell$ vary smoothly and agree with the flat result in
the limit, with no discontinuity at the switch between flat and non-flat
treatments. This continuity is a practical requirement for parameter estimation,
where a sampler must be able to cross $\Omega_K=0$ without seeing a step in the
likelihood. \todo{add the $\Omega_K\to0$ continuity figure (data in
nearflat\_omk\_pm1e-6 sweep)}

For genuinely non-flat models the source integration uses the Numerov path of
\cref{sec:bessel-numerov}, re-seeded periodically from Olver reference values; the
sampling in $l$ and the maximum wavenumber are adjusted for the geometric shift of
the acoustic peaks, with the near-flat table shortcuts used only when a
sampling-ratio test confirms the peak positions are close enough to the fiducial
flat sampling.
...
\code{lSampleBoost}, \code{lAccuracyBoost}, \code{IntTolBoost}), which empirically
...
rather than blockers. \todo{add the default-vs-reference residual summary figure
across the model grid}

This harness is also the objective function for the development methodology
described next: it is what tells us, deterministically, whether a proposed change
helps, leaves accuracy unchanged, or regresses some other model.

\section{Agentic co-development}
\label{sec:agentic}

Building and, crucially, validating methods at this level of specificity is
laborious, and almost all of the work described here was carried out by AI agents
with a human in the loop. The agents operated against the deterministic
\code{check_accuracy} harness of \cref{sec:validation} as an objective function,
using a small toolchain packaged as reusable agent \emph{skills} and
\emph{subagents}, so the same procedure could be invoked repeatably: an
orchestrating skill that isolates a single, physically scoped accuracy fix at a
time and retains sole authority over the patch; a diagnostic subagent that
reproduces one failure in a disposable copy of the code and drills from the broad
accuracy control down to the specific lines responsible, rebuilding before every
test; and an independent read-only subagent that audits each proposed change for
scope, gating and physical motivation. The human role was to set
the targets, supervise the physics, and review and accept the final changes.
...
AL is supported by the UK STFC grant ST/X001040/1. I thank Jens Chuba and Yacine Ali-Haimoud for recombination advice,
