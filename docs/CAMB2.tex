...
\emph{custom-precision} numerical methods, hand-fitted and exhaustively calibrated
...
spend accuracy where it matters, and prove it with a reproducible test -- runs
...
practical to hand-fit and exhaustively validate this many small numerical changes
...
Dormand--Prince 4(5) pair~\cite{DormandPrince:1980}, keeping the same
...
Writing $\hat\ell=\sqrt{l(l+1)}$ for the Langer-corrected multipole and
...
K}}\ll1$. The Bessel factor for multipole $l$ peaks near $\SK(\chi)\simeq\hat\ell/\nu$,
...
For flat and near-flat cases, the functions are splined from pre-computed tables of this function.
...
\subsection{The near-flat shifted-wavenumber shortcut}
...
source integrators also allow a calibrated high-$l$ near-flat branch where the
...
\midrule
Flat & table/spline of $\jl(q\chi)$ & accurate flat result \\
...
Non-flat range, $n_{\rm substep}\ge10$ per \\ sampled source & direct Olver at each source point & one Olver call $\approx$ 13 substeps; avoids phase drift \\
Low-$l$ / low-$\alpha$ & Airy/open-small-$\nu$ fallbacks, then \code{phi_recurs} & asymptotics not valid \\
\bottomrule
\end{tabularx}
\caption{Source-integration hierarchy. Near-flat paths require their calibrated
accuracy gates to pass (\cref{app:bessel-gates}).}
\label{tab:hierarchy}
\end{table*}
...
model's. The template then remains accurate enough for the small spline correction
when the peaks are displaced from the fiducial position.
...
\textsc{CAMB} current default & 5.3 & 1.3 \\
\textsc{CAMB} v1.x, same high-\(k\) support & 90 & 21 \\
\textsc{CLASS} default & 15 & 3.6 \\
\textsc{CLASS} \code{cl_permille} defaults & 27 & 6.4 \\
%\textsc{CLASS} default, same physical reach & 15 & 3.6 \\
%\textsc{CLASS} \code{cl_permille}, same physical reach & 27 & 6.1 \\
\hline
\end{tabular}
}
...
geometric-mean CPU time falls from $90.1\,$s for \textsc{CAMB} v1.x to
$5.30\,$s at $l_{\max}=4000$, a factor of $17$ at higher target accuracy, and
the near-flat $|\Omega_K|\le10^{-3}$ models ($4.82\,$s geometric mean) cost only
modestly more than the exactly flat case ($3.74\,$s).
...
lensing observables that \textsc{CAMB} also computes. The unboosted defaults are
designed to reach the $10^{-3}$ level for the interior lensed CMB and the
...
\(\alpha_g=\nu/l\),
\begin{equation}
  \alpha_g\ge\alpha_{\rm open}(l),\qquad
...
\Cref{fig:bessel-accuracy-domains} shows the point-call domains and flags the worst-case validation errors over a
wide region of hyperspherical bessel parameter/argument space. Small $nu/\ell$ and other domains included here are rarely encountered in the CMB calculation, but the \code{phi_olver} function is tested to be valid over the full range of its arguments for completeness.
...
Each dot is one sampled call \((K,l,\nu,\chi)\), colored by the branch selected
...
land on the same plotted point; overlapping Airy and recurrence colors therefore
