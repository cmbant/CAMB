...
\begin{figure*}[t!]
  \begin{center}
  \includegraphics[width=0.88\textwidth]{changelog/lensed_cls_validation/lensed_cls_validation_residuals_planck_recfast_cl_ref_ee_gt3000_mnu0p06.pdf}
  ...
  numerical like-for-like accuracy rather than physical accuracy. The gray band
...
\cref{sec:validation} as the validation target for final \code{CAMB} outputs.
...
code changes are targeted, physically reasonable, and guard against fixes that fix the diagnostic but not the underlying cause. A subagent reproduces one failure in a disposable copy of the code and drills from the broad
...
summarizes representative four-thread timings on a typical laptop.  The flat
runs use a Planck-2018 \(\Lambda\)CDM model with \(m_\nu=0.06\,{\rm eV}\) and
lensed spectra requested to \(l_{\max}=4000\).  The current default uses
\(\max(k\eta_0)=90000\), or \(k_{\max}=6.36\,{\rm Mpc}^{-1}\).  The old
\textsc{CAMB} v1.x default was faster only because it used the historical low
lensing cutoff \(\max(k\eta_0)=10375\).  When the old code is run with the same
high-\(k\) support as the new default, it is slightly slower than the current
code and less accurate.
...
Case & CPU [s] & wall [s] \\
\hline
\multicolumn{3}{c}{Flat \(l_{\max}=4000\), Planck-2018 model} \\
\textsc{CAMB} current default & 4.0 & 1.0 \\
\textsc{CAMB} v1.x, same high-\(k\) support & 4.4 & 1.1 \\
\textsc{CAMB} v1.x, historical low-\(k\) default & 2.4 & 0.58 \\
\textsc{CLASS} default, native reach, \(L=4000\) & 10 & 3.4 \\
\textsc{CLASS} \code{cl_permille}, native reach, \(L=4000\) & 11 & 3.7 \\
\textsc{CLASS} default, high-\(k\), \(L=6000\to4000\) & 15 & 4.6 \\
\textsc{CLASS} \code{cl_permille}, high-\(k\), \(L=6000\to4000\) & 17 & 5.1 \\
\hline
\multicolumn{3}{c}{Non-flat 11-model geometric mean} \\
\textsc{CAMB} current default & 5.1 & 1.2 \\
\textsc{CAMB} v1.x, same high-\(k\) support & 68 & 16 \\
\textsc{CLASS} default & 14 & 3.4 \\
\textsc{CLASS} \code{cl_permille} defaults & 24 & 5.8 \\
%\textsc{CLASS} default, same physical reach & 15 & 3.4 \\
%\textsc{CLASS} \code{cl_permille}, same physical reach & 24 & 5.5 \\
\hline
\end{tabular}
}
\caption{Representative four-thread timings for lensed CMB calculations on a
typical laptop.  The native-reach \textsc{CLASS} rows use CLASS's own default
CMB source cutoff and, for the flat case, no lensed-spectrum margin.  The
high-\(k\) flat \textsc{CLASS} rows raise the CLASS CMB source cutoff to match
the current \textsc{CAMB} default, and compute lensed spectra to \(L=6000\)
before truncating to the requested \(l_{\max}=4000\).  This changes the CLASS
...
\cref{sec:bessel-curvature}.
% the ``same physical reach'' rows use the
%corresponding high-reach CLASS lensing setting for that calculation.
}
\label{tab:performance-timings}
\end{table}

The \textsc{CLASS} timing depends sensitively on whether the calculation is set
up as a converged high-\(L\) comparison.  If CLASS is run only to the requested
\(l_{\max}=4000\), the lensed-spectrum convolution edge dominates the residual:
...
\(1.8\times10^{-2}\) at \(l=4000\).  Once this edge is removed, changing from
CLASS's default source cutoff to the high-\(k\) cutoff used in the table changes
the \code{cl_permille} spectra by \(2.2\times10^{-3}\) in the full \(TT,EE,TE\)
envelope at \(l=4000\).  These lensing-related settings mainly affect the last
part of the damping tail, \(l\gtrsim3000\), but they are needed for a clean
high-\(L\) accuracy comparison.
...
at matched lensing support.  For the non-flat models, the revised
hyperspherical-Bessel calculation also removes the large cost of running the
previous version with accuracy settings that maintain the same target accuracy.
...
\Cref{app:accuracy} describes the accuracy parameters and default tolerances.
The default settings are designed to be sufficient for next-generation CMB experiments up to Advanced SO precision. Tools are provided to easily tune parameters if higher accuracy is needed.
