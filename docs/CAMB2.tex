...
\begin{equation}
  \vartheta_{\rm samp}=\frac{r_s(z_{\rm drag})}{D_M(\tau_0)},\qquad
...
acoustic peaks line up with the model's. The \(l\)-sampling gate is one-sided,
matching the current code. If \(\peakscale\ge1-0.03\), the fiducial grid is kept;
wider-spaced peaks are then simply oversampled. If \(\peakscale<1-0.03\), the
peaks are compressed enough that the sampled \(l\)-grid is made denser, and the
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Here \(z_{\rm scale}\equiv (T_{\rm CMB}/T_{\rm COBE})(1+z)-1\), so the correction
...
it is straightforward to fudge \textsc{HyRec}'s Helium equation to match, and then give accurate agreement
...
\begin{equation}
  ...
  D_\nu\equiv\bar\rho_\nu-3\bar P_\nu .
\end{equation}
The density and pressure use a smooth fit for \(0.42<a m_\nu<70\), while
...
\textsc{hmcode}~\cite{Mead:2020vgs}). The non-linear power also scales the CMB lensing potential, and hence modifies the lensed CMB power spectra, especially on small scales.
...
tolerance up to interpolation errors at sparse sampling. The practical accuracy floor is set by nonlinear modelling uncertainty rather than by the linear numerics. Thanks to an author contribution, baryonic feedback on the
...
Yacine Ali-Haimoud for recombination discussions, Simons Observatory collaborators on
likelihood and theory, and CAMB GitHub issue and pull-request
...
agents (Claude Code, Codex and Copilot) under my supervision; I am responsible
