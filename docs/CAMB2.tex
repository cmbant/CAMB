...
% Drafted from docs/changelog/hyperspherical_bessels.tex as a base (Sec.~III and
% App.~A reuse its derivations). The body is set two-column for paper format; the
% appendices switch back to a full-width grid for the heavier display mathematics.
...
\begin{abstract}
The next generation of cosmological surveys demand CMB theoretical power spectra
that are accurate to around the $0.1\%$ level out to high angular multipoles. We
describe a substantial update to the \textsc{CAMB} code aimed at a fast physics-based calculation
to the required accuracy. We focus on the two products that drive most analyses: the lensed CMB power
spectra and the matter power spectrum. The new unboosted default settings are
designed to reach the $10^{-3}$ level for the interior lensed CMB spectra and for
the matter power spectrum across the quasi-linear regime. We give a new line-of-sight
integration scheme built on a flat-Bessel comparison (leading-order Olver)
mapping of the hyperspherical Bessel functions, which is exact in the flat limit,
smooth through the turning point, and delegates almost all of the special-function
work to a fast flat spherical Bessel routine; near-flat models reduce to a
rescaling of the flat argument, and a Numerov integrator advances
neighbouring source points cheaply between occasional reference evaluations. We
also describe a fast updated recombination model fit, a stabilized parameterized post-Friedmann dark-energy
evolution, CMB lensing accuracy work, and discuss the different possible definitions of the linear matter power spectrum in the presence of strongly non-linear sources from reionization. Accuracy is assessed
with a deterministic validation harness that compares default runs to
boosted references. Essentially all of the new code and algorithms were
co-developed with AI agents under human supervision. The non-flat routine illustrates a more general shift towards
\emph{custom-precision} numerical methods: implementations calibrated to exactly
the accuracy a problem needs on the grid it actually samples, rather than carrying
expensive fixed precision of general-purpose libraries.
\end{abstract}
...
the two pillars of precision cosmology, and both are computed for a given model
by Einstein--Boltzmann codes such as \textsc{CAMB}~\cite{Lewis:1999bs} and
\textsc{CLASS}~\cite{Blas:2011rf}. These codes solve the linearized perturbation
equations and project them onto observable angular power spectra using
...
\textsc{COSMICS}~\cite{Ma:1995ey,Bertschinger:1995ag} and the closely related
all-sky FRW treatments of the 1990s~\cite{Hu:1998mn}. \textsc{CMBFAST} was later
generalized to non-flat geometries using hyperspherical Bessel
functions~\cite{Zaldarriaga:1997va}, and \textsc{CAMB} itself originated as a
rewrite supporting closed as well as open and flat FRW models~\cite{Lewis:1999bs},
built on the covariant CMB formalism of Refs.~\cite{Hu:1998mn,Challinor:2000as}.
The accuracy required of them is set by the data. For the \textit{Planck} satellite, default settings tuned to
roughly the $0.1\%$ level were sufficient~\cite{Howlett:2012mh}. Ground-based CMB
experiments such as the Simons Observatory and its planned
extension~\cite{SO:2018sim}, together with the matter-power measurements from
\textit{Euclid}, DESI and the Vera Rubin Observatory, demand both higher
precision and a wider range of scales, and over a broader set of models.

The last systematic description of \textsc{CAMB}'s accuracy and sampling
machinery was given by Ref.~\cite{Howlett:2012mh}, in the context of
\textit{Planck}. Much has changed since then, and the most substantial changes
have no published description. This paper fills that gap. It is not a review of
the underlying physics, which is standard and documented
...
and concentrate on what is new.

We concentrate on the two outputs that dominate practical analyses, the lensed
...
routine~\cite{cambflatbessel}, which already carries the Airy transition, the
Debye tails and the oscillatory phase; the curved problem reduces to a coordinate
map and an amplitude. The method is exact when the universe is flat, smooth
through the turning point, and in near-flat models collapses to a rescaling of the flat Bessel argument and an
amplitude. The flat bessel mapping is very similar in spirit to the
mapping approximation used by~\cite{tram2013,lesgourgues-tram2013}, but more general and implemented differently.
...
is a design philosophy: away from general-purpose special-function libraries with
a fixed accuracy/speed trade-off, and towards \emph{custom-precision} numerical
methods, hand-fitted and exhaustively calibrated to exactly the accuracy a given
problem needs on the grid it actually samples. The hyperspherical Bessel routine
is the clearest example, but the same principle -- spend accuracy where it
matters, and prove it with a reproducible test -- runs through all of the changes.
The second is the process: almost all of this work was co-developed with AI tools.
 For more analytic work, LLMs were used to suggest and critique possible implementation algorithms or approximations, with iterative testing and cross-LLM/human validation and review.
For accuracy stability failures and bugs, agents with a human in the loop drove a deterministic validation harness to isolate and fix issues (\cref{sec:validation,sec:agentic}). The agents made it practical to
hand-fit and exhaustively validate this many code tweaks and alternative numerical methods, a labour that would
otherwise be prohibitive.

We begin with conventions and code notation (\cref{sec:conventions}), then
describe the line-of-sight and hyperspherical-Bessel calculation
...
performance (\cref{sec:performance}), and recommended settings
(\cref{sec:summary}). \Cref{app:bessel} collects the detailed Bessel mathematics
...
repeat them. The curvature conventions used by the Bessel routines are given at
the start of \cref{sec:bessel}.
...
source against a radial Bessel function~\cite{Seljak:1996is,Challinor:2000as}. In
...
non-flat generalization of $\jl$ \cite{abbott-schaefer,Kosowsky:1998nc,tram2013}.
...
$(kR_K)^2=\nu^2-K$ for scalars. In the flat limit $q=k=\nu/R_K$.
...
\begin{equation}
  ...
  \label{eq:reduced}
\end{equation}
The regular solution is fixed by $\phi_l^\nu\sim
\big[\textstyle\prod_{j=1}^l b_j\big]\chi^l/(2l+1)!!$ as $\chi\to0$, with
$b_j(K,\nu)=\sqrt{\nu^2-Kj^2}$; for $K=0$ this is the usual
$\jl(\nu\chi)\sim(\nu\chi)^l/(2l+1)!!$. For $K=+1$, $\nu$ is a positive integer
and the regular spectrum requires $\nu>l$.

Writing $\ell=\sqrt{l(l+1)}$ and $\alpha=\nu/\ell$, \cref{eq:reduced} becomes
$u''+\ell^2[\alpha^2-\SK^{-2}]u=0$, with a turning point where the bracket
vanishes,
\begin{equation}
  \SK(\chi_t)=\frac1\alpha=\frac{\ell}{\nu}.
  \label{eq:turning}
\end{equation}
Below $\chi_t$ the regular solution is exponentially small; above it the solution
oscillates. A direct WKB amplitude diverges at the turning point even though the
true function is finite, so any practical method must be uniform through this
transition. This large-$\ell$ turning-point language organizes the moderate- and
high-$l$ behaviour; for $l\le2$ and a few other corners the code uses the exact
recurrence of \cref{sec:bessel-recur} instead.

For most cosmologies of interest the curvature radius is far larger than the
horizon, so every source point has $\chi\lesssim\chi_0=\tau_0\sqrt{\abs{\mathcal
K}}\ll1$. The Bessel factor for multipole $l$ peaks near $\SK(\chi)\simeq\ell/\nu$,
...
factor~\cite{olver}. The textbook choice is the Airy equation, giving the uniform
WKB/Langer treatment used for hyperspherical Bessel functions by
Refs.~\cite{Kosowsky:1998nc}. Our choice is different
...
implementation is literally this composition: compute $z$ and $A$, call the flat
routine for $\jl(\nu z)$, multiply by $Az$, and divide by $\SK$ for the unreduced
value.
...
\begin{equation}
  Q_K(\chi)=I_0(\alpha z),\qquad z_t=\frac1\alpha=\frac{\ell}{\nu},
  \label{eq:action-map}
\end{equation}
...
$|f|^{-1/4}\cos(\ell Q-\pi/4)$ on the oscillatory side, with $\ell Q$ the action.
...
Matching the action fixes the phase; the amplitude follows from flux (Wronskian)
conservation. Differentiating \cref{eq:action-map} gives the local map
...
\begin{equation}
  ...
  \label{eq:amplitude}
\end{equation}
which keeps the Wronskian constant and reduces to the WKB $|f|^{-1/4}$ in the flat
...
\cref{eq:basic-olver} reduces \emph{exactly} to the flat result. This exactness in
the flat limit, and the smoothness through the turning point, are the two
properties that make the flat comparison the right choice; both are a consequence
of using $z\jl(\nu z)$, which solves the exact reduced flat equation, rather than
a local Airy function.

\subsection{The near-flat shifted-\texorpdfstring{$q$}{q} shortcut}
...
\begin{equation}
  \phi_l^\nu(K,\chi)\simeq \sqrt{F_0}\,\frac{\chi}{\SK(\chi)}\,\jl(q_{\rm eff}\chi),
  \qquad q_{\rm eff}=\nu F_0 .
  \label{eq:shifted-q}
\end{equation}
This is as cheap as a flat-table lookup: a flat $\jl$ evaluated at a shifted
argument $q_{\rm eff}\chi$, multiplied by $\sqrt{F_0}$ and the geometrical factor
$\chi/\SK$. Expanding, $q_{\rm eff}^2=\nu^2[1-h/3-2h^2/45-\dots]$, whose leading
...
roughly the $10^{-4}$ level across the open and closed validation grid. The
controlling combination is $\alpha\simeq\nu/l$: larger $\alpha$ means weaker
curvature corrections and a smaller turning radius. The first omitted Olver term
scales as $\alpha^{-4}$, which sets the broad $\alpha\simeq$ few boundary; the
source fillers also allow a calibrated high-$l$ near-flat branch where the
small-$\chi$ endpoint metric is tighter. Below the accepted regions the code
organizes the residual by endpoint metrics, then tries validated Airy and open
small-$\nu$ fallbacks before using the exact recurrence. The precise pointwise
decision order and the source-integration filler gates are given in
\cref{app:bessel-gates}.

For source integration the hierarchy is summarized in \cref{tab:hierarchy}. The
overall accuracy is controlled jointly by the pointwise Olver gates, the near-flat
filler gates, and the Numerov sampling described next, not by the raw asymptotic
formula alone.

\begin{table*}[t]
\centering
\small
\begin{tabularx}{0.92\textwidth}{Z{0.78}Z{1.08}Z{1.14}}
\toprule
Regime & Computation & Reason \\
\midrule
Flat & table/spline of $\jl(q\chi)$ & exact flat result \\
Near-flat, tiny curvature & constant-map shifted-$q$ lookup, \cref{eq:shifted-q} & fastest limit of small-$\chi$ Olver \\
Near-flat, small-$\chi$ range & lookup at $\nu\chi F$, weight $F/\sqrt D$ & retains next curvature terms \\
General non-flat range & start from Olver, propagate by Numerov & avoids repeated Olver calls \\
Non-flat range, $n_{\rm sub}\ge10$ per step & direct Olver at each source point & one Olver call $\approx$ 13 substeps; avoids phase drift \\
Low-$l$ / low-$\alpha$ & Airy/open-small-$\nu$ fallbacks, then \code{phi_recurs} & asymptotics not valid \\
\bottomrule
\end{tabularx}
\caption{The source-integration approximation hierarchy. Each near-flat path is
accepted only when its calibrated accuracy gate passes (\cref{app:bessel-gates}).}
\label{tab:hierarchy}
\end{table*}
...
\begin{multline}
  y_{n+1}=
  \Big[2\big(1+\tfrac{5\eta^2Q_n}{12}\big)y_n
  -\big(1-\tfrac{\eta^2Q_{n-1}}{12}\big)y_{n-1}\Big] \\
  {}\times\left(1-\tfrac{\eta^2Q_{n+1}}{12}\right)^{-1},
  \label{eq:numerov}
\end{multline}
where $\eta$ is the (signed) substep. Being a two-step recurrence it needs two
...
steps, reduced at higher accuracy -- the code re-seeds by recomputing $\SK,\CK$
directly and re-evaluating the two Numerov starting values from fresh Olver calls.
...
residual inter-seed Numerov phase drift ($C_\ell$ shifts $\lesssim2\times10^{-5}$).
...
Before stepping the ODE, the code first tries the two near-flat fillers of
\cref{tab:hierarchy}, which fill an entire range directly from the flat Bessel
table when their gates pass. These avoid stepping the curved ODE at all and, just
as importantly, vary continuously into the exact flat result as $\mathcal K\to0$,
...
recurrence in \code{phi_recurs}. This recurrence machinery is essentially
...
Because the recurrence is standard, we relegate the formulae and the detailed
\textsc{CLASS} comparison to \cref{app:bessel-recur}.

\subsection{Why the flat comparison helps}
\label{sec:bessel-why}

A first-order Langer approximation maps the curved equation onto an Airy function,
uniform only at the level of the universal local turning-point shape. Our map
instead sends
\begin{equation}
  \text{curved Bessel}\;\to\;\text{flat spherical Bessel at }z(\chi),
\end{equation}
and the flat spherical Bessel routine already contains its own Airy transition,
Debye tails, oscillatory phase and low-order safeguards. The curved calculation
therefore only has to describe how curvature changes the effective coordinate and
amplitude, which is a smooth, slowly varying problem. It is also why the
shifted-$q$ limit is so natural, and why the whole construction is exact in the
flat limit: the comparison object is the exact flat solution, not a local model of
it. The remaining error is the leading omitted Olver (Schwarzian) term of
\cref{eq:olver-residual}, which is parametrically small in the gated regime.
...
not resolve the acoustic peaks, so for $TT$, $EE$ and $TE$ we instead
...
The ingredient we add here is to shift the template to the model's acoustic scale
before taking the difference. With $s$ the ratio of the model's acoustic scale to
the template's -- set from the sound-horizon angle, the same geometric scale used
by the near-flat source-integration gate of \cref{app:bessel-gates} -- the
template is evaluated at $\ell/s$ so that its acoustic peaks line up with the
model's. The residual then stays smooth even when the peaks are displaced from the
fiducial, so a model with a different acoustic scale can reuse the same sparse
$\ell$ sampling instead of needing a denser grid; the sampling step is scaled by
$s$ for the same reason. Both the template shift and the step scaling are applied
only once $s$ departs appreciably from one, so flat and near-fiducial models are
left unchanged. This keeps the high-$\ell$ cost roughly fixed across the range of
acoustic scales of interest.

\subsection{Numerical performance}
\label{sec:bessel-curvature}

The non-flat path is also cheap. On a four-thread timing sweep of the 11
default-accuracy models
$\Omega_K=\{-0.03,-0.02,-0.01,-10^{-3},-10^{-5},0,10^{-5},10^{-3},0.01,0.02,0.03\}$
used in the validation set, the new implementation has a geometric-mean CPU time
of $5.14\,$s at $l_{\max}=4000$, against $68.3\,$s for the previous
\textsc{CAMB} v1.x code at the same effective lensing-potential accuracy -- a
factor $13.3$ speed-up (at higher accuracy). The corresponding \textsc{CLASS}~3.3.4 timings for the
same models, with its CMB-lensing full-Limber cutoff set to the same physical
reach as the new \textsc{CAMB} default lensing integration,
$k_{\max}=6.41\,\mathrm{Mpc}^{-1}$, are $14.5\,$s at default precision and
$24.3\,$s with \code{cl_permille} precision. Within the new \textsc{CAMB} sweep
the near-flat cases with $|\Omega_K|\le10^{-3}$ have geometric-mean CPU time
$4.58\,$s, against $3.74\,$s for the exactly flat $\Omega_K=0$ case; the most
closed case ($\Omega_K=0.03$) takes $5.15\,$s and the slowest open case
($\Omega_K=-0.03$) takes $7.21\,$s. Curvature is therefore a modest overhead
rather than a separate slow path, and the source-integration and lensing work
parallelizes well over the 4--8 threads typical of interactive analysis runs.
Current CMB constraints concentrate close to flatness, where the overhead is
smallest.
...
\textsc{PRIMAT} calculation is higher by \(2.0\times10^{-4}\). Across the
representative range
\(0.0218\le\Omega_{\rm b}h^2\le0.0230\) and
\(-0.5\le\Delta N_{\rm eff}\le0.5\), the largest helium shift relative to the
2024 \textsc{PRIMAT} prediction is \(3.0\times10^{-4}\), and at fixed cosmological
parameters the lensed \(TT\) and \(EE\) spectra shift by less than
\(7\times10^{-4}\) for \(30\le\ell\le4000\). The corresponding deuterium changes
...
can give non-negligible shifts in spectra and likelihoods. We have therefore
revisited both parts of the default recombination calculation: the effective
recombination history used in the standard run, and the numerical integration of
the resulting equations. This subsection describes the physical calibration of
the effective model; the next subsection describes the new semi-stiff integrator
used to solve it.
...
from Ly$\alpha$ and by the two-photon $2s\to1s$ channel. Modern CMB calculations
...
net decay rate calibrated on full \textsc{HyRec}~\cite{Lee:2020obi}; it does not
introduce a comparably new helium calculation. For the helium radiative-transfer
modelling tested here, this makes \textsc{CosmoRec} the more complete baseline;
\textsc{HyRec-2} remains a valuable independent high-accuracy comparison with a
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Here \(z_{\rm scale}=(T_{\rm CMB}/T_{\rm COBE})(1+z)-1\), so the correction
...
normalization were jointly refitted.\footnote{The rounded fitted \textsc{CAMB}
\code{Recfast} parameters are \code{RECFAST_fudge}=1.125,
\code{RECFAST_fudge_He}=0.84724, \code{RECFAST_He_rate_correction}=T, and
$(\code{AGauss1},\code{AGauss2},\code{zGauss1},\code{zGauss2},\code{wGauss1},
\code{wGauss2})=(-0.1395,0.07299,7.281,6.767,0.1639,0.2786)$, with the default
\textsc{RECFAST} switches. The rounded helium-rate constants are
\((a_0,a_1,a_2,d_2,d_4,z_0,w)=(0.0781,0.899,-3.06,3.75,18.1,1968,774)\).}
...
minimal extension of the existing effective model. Direct \textsc{HyRec-2} can
also be used as a \textsc{CAMB} recombination backend, and retuning the same
fast \textsc{RECFAST} machinery to its helium prediction would be
straightforward if that reference were preferred.
...
\begin{figure*}[t!]
  ...
  \caption{Upper panel: the direct \textsc{CosmoRec} reference $x_e(z)$ and the
  peak-normalized visibility density per unit redshift and Silk damping
  integrand, shown on a logarithmic vertical scale. The visibility curve is
  ...
  maximum. The gray dashed vertical line marks $z_\ast$. The upper horizontal
  ...
  \textsc{RECFAST} approximation. The reference is the
...
As a quantitative CMB-impact diagnostic, the accuracy checking code uses a simple Gaussian
mean likelihood, applied to lensed $TT,EE,BB$ spectra. At each
multipole we use the covariance expected from cosmic variance on sky fraction
$f_{\rm sky}=0.6$, a 1.4 arcmin beam, and white noise levels of 2.6 and
3.67 $\mu{\rm K}$-arcmin in temperature and polarization, then sum the
quadratic spectral differences over $2\le \ell\le4000$.
This is a deliberately compact, conservative proxy for the more complete Simons
Observatory forecasting likelihoods~\cite{SO:2018sim,SOEnhancedLAT:2025}.
...
\(C_\ell\)-accuracy significance. The last number is the mismatch to the chosen
\textsc{CosmoRec} reference, and for \textsc{HyRec-2} is dominated by the less
detailed helium modelling. For the matter power spectrum all models agree at the
$< 2\times 10^{-5}$ level.
...
and any non-standard energy injection, varying constants, or other non-standard
...
Dormand--Prince solver. The Rosenbrock and Dormand--Prince tolerances and the
...
to half its ionized value at $z_{\rm re}$ up to the small effect of the smoothing
factor $s$ (equivalently the model is set through $\tau$); the power $p$ controls
the steepness (default $p=1$); and $s(z)=[1+w/(z-z_{\rm end})^2]^{-1}$ keeps the
derivative continuous at $z_{\rm end}$. Pinning the completion redshift near
...
\subsubsection{Reionization heating and low-redshift linear matter power}
...
\begin{equation}
  y_{\rm heat} =
  \operatorname{clip}\!\left[\frac{x_e-x_e^{\rm rec}}{1-x_e^{\rm rec}},\,0,\,1\right],
\end{equation}
where $x_e^{\rm rec}$ is the recombination residual at the same time. It then
...
\begin{equation}
  c_s^2 = \frac{5}{3}\,\frac{k_{\rm B}T_{\rm gas}}{\mu m_p}.
\end{equation}
Here $\mu$ is the effective mean molecular weight. This construction follows the
...
\begin{figure}[t]
  ...
  \caption{Fractional change in the $z=0,1,2$ matter power spectrum from enabling
...
series limits outside it. Here $m_\nu$ denotes the dimensionless mass
$m_\nu c^2/(k_{\rm B}T_{\nu0})$ used by the code, so the expansion variable is
$a m_\nu$. The density and pressure use a smooth fit for
$0.42<a m_\nu<70$, while the combination
$D_\nu\equiv\rho_\nu-3P_\nu$, which enters
$\dot\rho_\nu=\mathcal H D_\nu$, uses a separate fit over
$0.3<a m_\nu<70$. Against direct Fermi--Dirac quadrature these fits reach a
maximum relative error below $10^{-4}$ in $\rho_\nu$, $P_\nu$ and $D_\nu$, which
is amply sufficient; they remove the table and its global state and are
marginally faster in the hot path.
...
$m_\nu c^2/(k_{\rm B}T_{\nu0})\gtrsim600$ (roughly $0.1\,{\rm eV}$), and to five
points above $m_\nu c^2/(k_{\rm B}T_{\nu0})\gtrsim4300$ for the heaviest transfer
cases.
...
velocity-integrated hierarchy of Lewis and Challinor~\cite{Lewis:2002nc},
implemented by the \code{Nu_trunc} scheme. Once the neutrinos are safely
non-relativistic, the default \code{Nu_best} path integrates the distribution into
a four-variable hierarchy for $\delta\rho_\nu$, $\delta P_\nu$, heat flux and
anisotropic stress, with higher velocity moments redshifting analytically.
Separately, \textsc{CAMB} uses the high-$k\tau$ subhorizon massless-neutrino
closure described for \textsc{CLASS}~\cite{Blas:2011rf}: the ultra-relativistic
fluid approximation replaces the full radiation hierarchy after $k\tau$ is large,
and the late radiation hierarchy is then switched off once the multipoles are
dynamically negligible (\code{tau\_switch\_no\_nu\_multpoles}). These schemes are
...
\begin{equation}
  ...
  \label{eq:ppf-gamma}
\end{equation}
For fixed source this is a relaxation equation: $\Gamma$ is driven at the rate
...
$\Gamma_{\rm qs}$, and avoids the matter-power and $C_\ell$ artifacts of a hard
cutoff. For a near-$\Lambda$CDM example the peak normalized dark-energy density
perturbation at $k=0.1\,\mathrm{Mpc}^{-1}$ falls from $\sim3\times10^{3}$ to
...
is the default PPF behaviour. \todo{add before/after $\Delta_{de}$ and
...
The lensed CMB spectra and the matter power spectrum are the two outputs we have
worked hardest to make accurate, because they dominate current and forthcoming
analyses.
...
non-perturbative isotropic factor $\exp[-l(l+1)\sigma^2/4]$ together with a
...
earlier treatment)\footnote{At low multipoles the reionization bump in $EE$
can cause instabilities in the apodized truncated correlation function calculation.
We apply a smooth taper that removes the low-$\ell$ bump to regularize the lensing
calculation. We do not attempt to accurately model the tiny linear signal of lensed
reionization rescattering, which the full angular range calculation slightly
overestimates due to the lower source redshift than the primary CMB.}. When accurate $BB$ is required -- for tensor B-mode analysis for
...
sampling and, for nonlinear spectra, by the nonlinear model
(\textsc{halofit}~\cite{Takahashi:2012em} or
\textsc{hmcode}~\cite{Mead:2020vgs}). The validation work of
...
suite, which now passes at the target tolerances across the model grid. Baryonic
feedback on the small-scale matter power can now also be included with the \textsc{SP}(k)
model~\cite{Salcido:2023etq}.

\section{Validating accuracy}
\label{sec:validation}

Every accuracy claim in this paper rests on a deterministic validation harness,
\code{camb.check_accuracy}, which compares a default run to a boosted reference and
reports the fractional differences per output, multipole or wavenumber range. The
reference doubles the main accuracy controls (\code{AccuracyBoost},
\code{lSampleBoost}, \code{lAccuracyBoost}), which empirically
brackets the converged result. The default, unboosted settings are designed so
that the interior lensed CMB spectra and the matter power spectrum in the
quasi-linear regime agree with this reference at the $10^{-3}$ level -- the
accuracy relevant for extended SO and the matter-power surveys. The per-output
acceptance tolerances that encode this are listed in \cref{app:accuracy}: in
brief, $10^{-3}$ for the lensed $TT/EE/TE$ over $600\le\ell<3500$ (looser at the
ends and in the high-$\ell$ tail), $5\times10^{-3}$ for the lensing potential over
$5\le L<2500$, and $10^{-3}$ for the linear matter power over the quasi-linear
range $5\times10^{-3}<k/h<2$ (in $h\,\mathrm{Mpc}^{-1}$; looser outside it, and in
the nonlinear regime where the nonlinear model sets the floor). A pointwise
relative comparison is deliberately avoided where the spectra cross zero.

Running this harness over a grid of more than a hundred parameter files -- spanning
curvature, massive neutrinos, dynamical dark energy, nonlinear and transfer-only
configurations, and a range of $l_{\max}$ -- the current default settings pass at
the target tolerances in essentially all cases, with the residual failures
confined to characterized endpoint and extreme-tail regions (the very edge of a
requested $l_{\max}$, and the high-$\ell$ tail of $21\,$cm configurations) rather
than to interior accuracy. We treat those edge cases as documented limitations
...
between the flat and non-flat treatments. We verify this directly: sweeping
$\Omega_K$ through zero at fixed angular acoustic scale, the lensed $C_\ell$ and other spectra vary smoothly and
approach the flat result to well wtihin tolerance as $\Omega_K\to0$.


\Cref{fig:lensed-cl-validation} shows a representative high-resolution lensed
CMB test at \(l_{\max}=4000\). To isolate numerical accuracy from changes in
...
physical cutoff \(k_{\max}\eta_0=144000\), fixed \(Y_{\rm He}=0.245\), and the
...
\begin{figure*}[t!]
  \begin{center}
  \includegraphics[width=0.88\textwidth]{changelog/lensed_cls_validation/lensed_cls_validation_residuals_planck_recfast_cl_permille_boosted_ee_gt3000.pdf}
  ...
  reference. Up to \(\ell=3000\) the plotted quantity is the envelope
  ...
  version 3.3.4 with \code{cl_permille} precision and a targeted boosted
  \code{cl_permille} variant; to avoid lensing-convolution edge effects the \code{cl_permille} curve is computed to
  \(\ell=8000\), while the boosted  \code{cl_permille} curve is computed to \(\ell =6000\). Both
  codes use Planck-era RECFAST with fixed \(Y_{\rm He}=0.245\), no massive
  neutrinos, standard HMCode
  2020 with no feedback, and matched \(k_{\max}\eta_0\), so the comparison as far as possible tests
  numerical like-for-like accuracy rather than physical accuracy. A corresponding
  check with the standard minimal mass \(\sum m_\nu=0.06\,{\rm eV}\) gives a
  similar result. The gray band
...
\section{Agentic co-development}
\label{sec:agentic}

The custom-precision approach of this paper is laborious to build and, more so, to
validate, and almost all of that work was carried out by AI agents with a human in
the loop, against the deterministic \code{check_accuracy} harness of
\cref{sec:validation} as the validation target. In the common case the agents
worked directly: generating diagnostic scripts that compare a routine against a
high-accuracy reference, scan the region a gate will actually accept, and propose a
local fix. Most of the hyperspherical Bessel calibration was done this way -- for
example tuning the continuous \(\nu/l\) cutoff for the full-Olver fast path to the
point where a handful of accepted errors between \(10^{-4}\) and \(2\times10^{-4}\)
buy a large fraction of fast-path evaluations, a measured tradeoff chosen for the task in hand.

For regressions whose cause was not obvious, the workflow separated the roles to
keep fixes physically local and guard against overfitting: a diagnostic subagent
reproduces one failure in a disposable copy of the code and drills from the broad
accuracy control down to the responsible lines; an orchestrating skill holds sole
authority over the patch and changes one issue at a time; and an independent
read-only subagent audits the result for scope, gating and physical motivation. The
...
allows. The skills, subagents, and a dated changelog per fix are kept in the
repository as part of the reproducible record.

The agents were consistently good at isolating a minimal change that restores the
harness, but markedly less reliable at finding the \emph{right} one: a patch could
...
judgement, which current models are not yet good at, so the work was iterative --
multiple rounds of search, with the human rejecting passing-but-unphysical
candidates and redirecting the next pass. The harness makes a regression visible
and reproducible, but does not by itself certify that a fix is physical; that,
along with setting the accuracy targets and accepting the final changes, remained a
human responsibility.

The markdown files defining the \code{isolate-accuracy-fix} skill and subagents,
along with a \code{classy-comparisons} skill guiding \code{CLASS} installation and like-with-like runs, are provided with the source code.
...
cheap Numerov propagation of neighbouring source points. Around this we have
faster recombination and neutrino-background evaluation, a stabilized PPF
dark-energy evolution, and recalibrated lensing and curvature accuracy, all
validated by a deterministic harness across a large grid of models. The unboosted
defaults are designed to reach the $10^{-3}$ level for the interior lensed CMB and
the quasi-linear matter power (\cref{app:accuracy}).

\Cref{app:accuracy} collects the accuracy parameters and our recommended settings.
\todo{add the per-experiment recommended-settings table (the single most useful
table for readers): default vs.\ high-accuracy columns for SO/extended-SO lensed
CMB and for matter power, with the resulting validated tolerance.}
...
validated deterministically -- to become an increasingly common way to meet the
accuracy demands of precision cosmology.

\begin{acknowledgments}
AL is supported by the UK STFC grant ST/X001040/1. I thank Jens Chluba and Yacine Ali-Haimoud for recombination advice,
Simons Observatory collaborators on likelihood and theory, and various CAMB github issue and PR contributors who
reported errors or contributed new code or patches. The
algorithms and code described here were co-developed with LLM chats and AI coding agents
(Codex, Copilot and Claude Code) under the author's supervision; the author is responsible for the physics, the design of
the validation, and the final review of all changes.
\end{acknowledgments}
...
\begin{equation}
  \frac{\dd^2w}{\dd z^2}+\big[\ell^2 f_0(z)+\Psi(z)\big]w=0,\qquad
...
\begin{align}
  ...
  \label{eq:F-smallchi}\\
  D&\equiv\frac{\dd z}{\dd\chi}=1-h\left[\frac16+\frac{12a+13h}{360}+\frac{240a^2+444ha+737h^2}{45360}\right],
  \label{eq:D-smallchi}
\end{align}
giving the local approximation
$\phi_l^\nu\simeq (\chi F/\SK)\,D^{-1/2}\jl(\nu\chi F)$. Setting $a=0$ recovers
$F_0=D_0$ of \cref{eq:F0}, the constant-map (shifted-$q$) limit.
...
the more accurate $\ell=\sqrt{l(l+1)}$ curvature scale. If that branch is not
used, open models accept the full action-map value whenever
\[
  \frac{\nu}{l}\ge\alpha_{\rm open}(l),\qquad
...
\[
  \frac{\chi}{2(\nu-l)}\le7.0\times10^{-3}.
\]
Otherwise the fallback sequence first tries the second-order Airy one-point
approximation for validated high-$l$ points ($l\ge100$ open, $l\ge150$ closed)
and, in open models, a narrow small-$\nu$ approximation before calling
\code{phi_recurs}. These thresholds are calibrated on peak-normalized error,
keeping the raw Olver envelope near $10^{-4}$ on the validation grid.
...
metric $l^2\chi^7/\nu<5\times10^{-2}$. The shifted-$q$ filler additionally
requires the constant-map error estimate
$\epsilon_{\rm shift}<10^{-3}/B_{\rm acc}$, where, with $t=\alpha\chi$,
\begin{equation}
  \epsilon_{\rm shift}=\frac12\frac{\ell\,t_{\max}^3}{90\alpha^4}+\frac{t_{\max}^2}{180\alpha^4}
  \label{eq:shift-error}
\end{equation}
combines the leading omitted argument shift and amplitude correction at the range
endpoint. A bookkeeping sampling-ratio test ($|s_{\rm peak}-1|\le0.03$, comparing
the acoustic-peak sampling to a fiducial flat one) gates whether the flat-table
machinery is reused at all.

\subsection{Recurrence and the \textsc{CLASS} comparison}
\label{app:bessel-recur}

The exact seeds are $\phi_0^\nu=\sin(\nu\chi)/(\nu\SK)$ and
$\phi_1^\nu=[\cotK\chi\,\sin(\nu\chi)/\nu-\cos(\nu\chi)]/(\SK\,b_1)$ (with Taylor
...
$\nu/l$ family when $\nu/l\le2.1\times10^{-3}$ and $y_{\rm open}\ge0.9$.
...
\section{Accuracy parameters and recommended settings}
\label{app:accuracy}

The user-facing accuracy is controlled by a small number of boost parameters, each
multiplying one aspect of the calculation: \code{AccuracyBoost} (overall sampling
and step sizes), \code{lSampleBoost} (density of computed multipoles before
interpolation), \code{lAccuracyBoost} (depth of the Boltzmann hierarchies),
\code{IntTolBoost} (ODE integration tolerances), \code{BackgroundTimeStepBoost}
(background and recombination grid), and \code{lens_potential_accuracy} (which
mainly raises $k_{\max}$ for the lensing and transfer integration). All default to
one, except \code{lens_potential_accuracy}, which by default is chosen
automatically from the requested $\ell_{\max}$ as $\max(4,(\ell_{\max}-1500)/500)$
(\cref{sec:lensing}); increasing any of them tightens the result towards the
converged reference.

The boosts form a hierarchy, not a set of independent line-by-line tolerances.
\code{AccuracyBoost} is the broad umbrella factor: it scales most of the
lower-level controls below it, such as time sampling, integration tolerances,
source sampling, and lensing sampling. By contrast, \code{lSampleBoost} and
\code{lAccuracyBoost} are largely orthogonal high-level controls for output
$C_\ell$ interpolation and Boltzmann-hierarchy truncation. The lower-level
boosts can be tuned directly, but each may still feed several related numerical
choices in the code. A useful workflow is therefore to use the high-level boosts
in \code{check_accuracy} to expose instabilities and coupled failures, then
drill down to the smallest lower-level control or code change that fixes the
problem. Production analyses should rarely need \code{AccuracyBoost}>1; if the
defaults are insufficient, targeted boosts usually give much better performance
than increasing the global factor.

The validation reference used by \code{check_accuracy} doubles
\code{AccuracyBoost}, \code{lSampleBoost}, \code{lAccuracyBoost} and
\code{IntTolBoost} together, which empirically brackets the converged result. The
default, unboosted settings are designed so that the lensed CMB and matter power
pass maximum-error tests at the $10^{-3}$ level over the range relevant to
extended SO and to the matter-power surveys, with no boost. \Cref{tab:tol} lists
...
$BB$ & $2\le\ell<1000$ & $5\times10^{-3}$ \\
 ...
 & $\ell\ge6000$ & $2\times10^{-2}$ \\
\midrule
Matter power & $k/h<5\times10^{-3}$ & $3\times10^{-3}$ \\
(linear) & $5\times10^{-3}<k/h<2$ & $1\times10^{-3}$ \\
 & $k/h>2$ & $3\times10^{-3}$ \\
\midrule
Matter power & $k/h<1$ & $3\times10^{-3}$ \\
(nonlinear) & $1<k/h<10$ & $5\times10^{-3}$ \\
 & $k/h>10$ & $1\times10^{-2}$ \\
\bottomrule
\end{tabular}
...
boosted reference, not RMS differences over the range; $k/h$ is in
$h\,\mathrm{Mpc}^{-1}$. The $10^{-3}$ entries -- the interior lensed CMB and the
quasi-linear matter power -- are the design target for extended SO and the
...
model sets the floor.}
\label{tab:tol}
\end{table}

To put these maximum-error tolerances on a likelihood scale, we evaluated the
same SO-like Gaussian mean likelihood used by the accuracy-checking code, with
the Planck-2018 best-fit spectra as the fiducial model and summed over
$2\le\ell\le4000$. This is a pessimistic envelope calculation, not a model for
the usual error distribution. If the errors in $TT$, $EE$, normalized $TE$ and
...
For analyses that need more than the default -- high-$\ell$ delensing, or matter
power deep into the nonlinear regime -- the boosts above should be increased
together. \todo{give a concrete recommended (\code{AccuracyBoost},
\code{lSampleBoost}, \code{lAccuracyBoost}, \code{lens_potential_accuracy}) tuple
per use case, with the resulting wall time and validated tolerance, once the
performance numbers in \cref{sec:performance} are finalized.}
