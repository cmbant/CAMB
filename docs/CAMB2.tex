...
\begin{abstract}
Upcoming CMB observations demand lensed theoretical power spectra
...
\cref{app:bessel-gates}. \Cref{fig:bessel-accuracy-domains} shows the resulting
point-call domains and flags the worst-case validation errors.
...
\caption{Hyperspherical-Bessel point-call domains in the current dispatcher.
Each dot is a sampled validation-grid point, colored by the branch used by
\code{phi_olver}; errors are measured against \code{phi_recurs} and normalized by
the first peak amplitude. Yellow triangles mark cases whose maximum sampled
error lies between \(10^{-4}\) and \(2\times10^{-4}\); no sampled case exceeds
\(2\times10^{-4}\). The dashed lines show the raw-Olver endpoint gates. In the
closed panel \(d\equiv\nu-l\); the dotted guides mark the large-\(l\) onset of
the closed Airy fallback and the value of \(d\) above which the raw-Olver metric
gate covers the full folded interval \(0\le\chi\le\pi/2\). The open small-\(\nu\)
branch applies only in the open panel.}
...
factor $13.3$ speed-up at matched accuracy. The corresponding \textsc{CLASS}~3.3.4 timings for the
...
normalization were jointly refitted.
%\footnote{The rounded fitted \textsc{CAMB}
...
dynamically negligible. These schemes are
...
tolerance up to interpolation errors at sparse sampling. The practical accuracy floor is set by the choice
of nonlinear model rather than by the linear numerics. Baryonic feedback on the
...
accuracy relevant for Advanced SO and the matter-power surveys. The per-output
...
range $5\times10^{-3}<k_h<2$, where
$k_h\equiv k/(h\,\mathrm{Mpc}^{-1})$ is the numerical wavenumber in
$h\,\mathrm{Mpc}^{-1}$ units (looser outside it, and in the nonlinear regime
where the nonlinear model sets the floor). A pointwise
...
standard \textsc{CAMB} test suite (\code{fortran/tests/run_tests.py}, driven by
\code{CAMB_test_files.py}) regenerates a broad grid of scalar, tensor, lensed,
transfer and matter-power outputs and compares them against precomputed saved
reference results to fixed numerical tolerances. The two checks are
...
Yacine Ali-Haimoud for recombination advice, Simons Observatory collaborators for
likelihood and theory discussions, and CAMB GitHub issue and pull-request
...
agents (Codex, Copilot and Claude Code) under my supervision; I am responsible
...
does not apply.
Operationally, whenever the leading Olver gate fails, the fallback sequence first tries the second-order Airy one-point
...
Advanced SO and to the matter-power surveys, with no boost. \Cref{tab:tol} lists
...
(linear) & $5\times10^{-3}<k_h<2$ & $1\times10^{-3}$ \\
 & $k_h>2$ & $3\times10^{-3}$ \\
\midrule
Matter power & $k_h<1$ & $3\times10^{-3}$ \\
(nonlinear) & $1<k_h<10$ & $5\times10^{-3}$ \\
 & $k_h>10$ & $1\times10^{-2}$ \\
\bottomrule
\end{tabular}
