...
tabulated argument when needed for shifted-\(q\) calls.  If \(s_{\rm peak}>1\),
...
branches that reuse the flat spherical-Bessel table: the shifted-\(q\) lookup
...
regions.  This is also why the shifted-\(q\) limit is tied to the main
...
The shifted-\(q\) approximation is the \(\chi\)-independent part of the local
Olver-to-flat expansion, \(q_{\rm eff}=\nu F_0\), with the simpler
