...
\begin{equation}
  ...
  \label{eq:amplitude}
\end{equation}
which keeps the Wronskian constant and reduces to the WKB \(|f|^{-1/4}\) in the
flat limit.
...
action-map Olver value throughout the sampled interval only when \(\nu/l\)
clears the calibrated continuous cutoff
\begin{equation}
...
the full present conformal time \(\tau_0\).  This \(\theta_{\rm drag}\) is
deliberately not the usual acoustic angular scale: the standard quantity divides
the sound horizon by the comoving angular-diameter distance \emph{to the drag (or
last-scattering) epoch}, whereas here the denominator is the distance over the
\emph{total} present conformal time.  It is used only as a cheap, close
approximate proxy for the acoustic peak positions in the sampling-ratio gate
below, not as a physical \(\theta_{\rm drag}\).  The sampling ratio is
\begin{equation}
...
\begin{enumerate}[leftmargin=*]
\item First, the local small-\(\chi\) approximation to the full action-map Olver construction must
