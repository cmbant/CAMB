...
precision.  (Their leading terms do agree with the analytic small-\(p\)
behaviour \(x\simeq1\mp\tfrac12 p^2+0.075\,p^4\) to about one part in
\(10^{5}\), which is a useful consistency check on the fits.)  Away from the
turning point the code switches to the natural
...
\((Q_K+\pi/2)^{-1}\).  The switch is placed at \(p=1.72\) on the evanescent
branch and \(p=1.97\) on the oscillatory branch, where the fitted and asymptotic
forms have equal error against an exact root solve of \(Q=t-\tanh t\) and
\(Q=\tan\theta-\theta\); the resulting worst relative errors in \(x\) over the
whole line are \(1.5\times10^{-6}\) and \(8.5\times10^{-9}\).  This keeps the Olver call fast: the map requires the
...
At the turning point both numerator and denominator vanish, but the limit is
finite.  Numerically, though, the ratio form \eqref{eq:amplitude} fails not only
at \(\chi_t\) but throughout a neighbourhood of it, where it is a quotient of two
catastrophically cancelled quantities; the code expands the map directly there
instead.  Substituting \(z=z_t+c_1\epsilon+c_2\epsilon^2\) with
\(\epsilon=\chi-\chi_t\) into the squared local map \eqref{eq:dzdchi} and
matching order by order gives, with \(C_t=\CK(\chi_t)=\sqrt{1-Kz_t^2}\),
\begin{equation}
  c_1=C_t^{1/3},\qquad
  c_2=\frac{3\alpha^2\left(C_t^{4/3}-C_t^{2}\right)-K}{10\,\alpha\,C_t^{2/3}},
  \label{eq:turning-limit}
\end{equation}
so that
\begin{equation}
  z=z_t+\epsilon\left(c_1+c_2\epsilon\right),\qquad
  A=\left(\frac{\dd z}{\dd\chi}\right)^{-1/2}
   =\frac{1}{\sqrt{c_1+2c_2\epsilon}},
  \label{eq:turning-expansion}
\end{equation}
which reduces to \(z-z_t\simeq C_t^{1/3}(\chi-\chi_t)\) and
\(A(\chi_t)=C_t^{-1/6}\) at leading order.  Keeping \(c_2\) matters in practice.
The window used by \code{compute_olver_z_amp} is
\(|\chi-\chi_t|\le10^{-4}\max(\chi_t,z_t)\), and in open space at small
\(\alpha\), where \(z_t\gg\chi_t\), that window is wide enough that a constant
amplitude \(A=C_t^{-1/6}\) across it is wrong by more than \(10^{-4}\) in
relative terms -- at the first peak, so directly at the level of the
peak-normalized target.  With the \(O(\epsilon)\) amplitude term, the relative
amplitude error at the window edge for \(K=-1\), \(\alpha=0.0955\) falls from
\(1.5\times10^{-4}\) to \(4\times10^{-8}\), and the relative error in \(z\) from
\(3\times10^{-8}\) to \(6\times10^{-12}\).  Since \(c_1\) is needed anyway,
\(c_2\) costs no transcendental evaluations, and \eqref{eq:turning-expansion}
replaces the fractional power \(C_t^{-1/6}\) by one square root.
...
The decision order in \code{olver_value}/\code{olver_reduced} is:
