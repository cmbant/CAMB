...
code therefore uses those metrics to decide when the leading full action-map
Olver value is still acceptable and otherwise falls back to stable recurrence.  The numerical
...
value whenever \(\nu/l\ge3\).
\item If \(\nu/l<3\), accept the full action-map Olver value only in the small
...
\end{equation}
Otherwise fall back to \code{phi_recurs}.
\end{enumerate}
