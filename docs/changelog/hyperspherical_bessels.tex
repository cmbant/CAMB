...
gate is given once in \eqref{eq:source-range-gate}; the shifted-\(q\) path then
...
near-flat range gate \eqref{eq:source-range-gate}.  Thus the shifted-\(q\)
...
\begin{equation}
  \frac{\chi}{2(\nu-l)}\le7.0\times10^{-3}.
\end{equation}
Otherwise fall back to \code{phi_recurs}.  This closed-space metric is still
applied at large \(\nu/l\): low-\(l\) and endpoint cases showed that an
unconditional \(\nu/l\ge3\) pointwise shortcut is too broad.
\end{enumerate}
For closed models the leading-Olver fallback is not a large uncontrolled slow
region.  The argument has already been folded into \(0\le\chi\le\pi/2\), so
failing the closed Olver gate implies
\[
  \nu-l < \frac{\chi}{2(7.0\times10^{-3})}\le 112.3 .
\]
This is only the region rejected by the leading Olver map.  Inside it the
fallback first tries the second-order Airy approximation for closed
\(l\ge150\), with the additional gate
\[
  \lambda(\beta^2-1)=
  \frac{\nu^2-\lambda^2}{\lambda}
  =
...
not to the cosmological scale factor \(a\).  Define the sampling proxy
\begin{equation}
  \vartheta_{\rm samp}
...
When this bookkeeping gate passes, the table paths still have to pass their
accuracy gates.  Both source table fillers first require the common near-flat
range gate
\begin{equation}
  \left[\frac{\nu}{l}>2.5\quad\hbox{or}\quad
  \left(l\ge50\ \hbox{and}\ \frac{\nu}{l}>1\right)\right],\qquad
...
\begin{enumerate}[leftmargin=*]
\item If the common range gate passes, \code{UseSmallChiNearFlatIntegration} may fill the range using
\(j_l(\nu\chi F)\) and the \(F/\sqrt D\) weight.
\item The shifted-\(q\) filler uses the same common range gate, and then adds the
...
shifted-\(q\) is used only when the constant-map Olver limit is valid and the
...
integrated source weights.  If neither table filler passes, the code uses the
general non-flat route:
\code{u_olver} for seed values, Numerov for nearby samples, and
\code{phi_recurs} only where the pointwise Bessel gate has selected it.
...
oscillations to average the drift away.  The minimum interval after accuracy
scaling is 25 substeps.  This bounds the accumulated error without paying for
...
Low-order, low-\(\alpha\), or closed finite-endpoint fallback & \code{phi_recurs} & exact recurrence is safer; high-\(l\) closed modes left after Airy have very small \(n=\nu-l-1\). \\
\bottomrule
\end{tabularx}
\end{center}
...
The current non-flat Bessel calculation combines a leading-order Olver-to-flat map with exact recurrence fallbacks and source-integration accelerations.  The map sends the reduced equation \(u''+[\nu^2-l(l+1)/\SK^2]u=0\) to the flat comparison solution \(zj_l(\nu z)\), with the Jacobian amplitude above, so it is uniform through the turning point and exact for \(K=0\).  In source integration, CAMB uses this point approximation mainly to seed Numerov stepping, while near-flat ranges can be filled from flat Bessel tables.  The shifted-\(q\) approximation is the \(\chi\)-independent part of the local Olver expansion, \(q_{\rm eff}=\nu F_0\), with the simpler \(q^2=\nu^2-Kl(l+1)/3\) formula retained only as a first-order explanatory limit.
...
The main text describes the approximation used most often in the current
hyperspherical-Bessel path: map the curved equation to a flat spherical Bessel
equation.  There is also a closely related one-point Airy form.  It is useful
near a turning point, and it is the form used by the optimized second-order
Airy code.  This appendix derives that approximation in a self-contained way.
The aim is to show what the second-order terms mean mathematically before
describing how they are evaluated cheaply.
...
\end{equation}
The symbol \(a_l\) is local to this appendix.  At large \(\chi\),
...
\end{equation}
the comparison phase matches \eqref{eq:app-smallnu-tail-phase} if
\begin{equation}
  \log\Lambda_{l\nu}
...
\end{equation}
No extra Liouville amplitude is applied.  This is a calibrated pointwise
replacement: the action-matched argument corrects the leading phase and
turning-region location, while the normalization remains that of the
phase-matched Macdonald comparison solution.
...
The small-\(\nu\) branch is open-space only.  The intended peak-normalized
validity region is
\begin{equation}
  l>20,\qquad
  \nu\le
...
\end{equation}
Outside this calibrated corner the ordinary Olver, Airy, or recurrence
...
Lanczos phase calculation.  The action-matched argument uses
...
\(\log(x/2)\) whenever possible.  If \(\log(x/2)>120\), it returns zero; if
