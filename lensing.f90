...
public lens_Cls, lensing_includes_tensors, lensing_method, lensing_method_flat_corr,&
      lensing_method_curv_corr,lensing_method_harmonic
contains
  ...
  do j=max_lensed_ix,1,-1  
     ...
     do j1 = 1, max_j_contribs
        ...
        if (DoPol) then
          ...
          call rec3jj(a3j(almin),dble(l1),dble(al),0._dl,2._dl)
...
subroutine rec3jj(thrcof,l2,l3,m2,m3)
  ...
  implicit none
  real(dl), intent(in) :: l2,l3, m2,m3
  real(dl) :: l1, m1, l1min,l1max, newfac,lmatch
  real(dl), dimension(*) :: thrcof
  integer ier
  real(dl), parameter :: zero=0._dl, eps=0.01_dl, one=1._dl
  real(dl) :: srtiny, sum1, tiny, oldfac, a1, a2, c1, dv, denom, c1old, &
