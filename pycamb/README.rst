...
For an introduction to CAMB see the main `home page <https://camb.info/>`_ , and the
`CAMB python example notebook <https://camb.readthedocs.org/en/latest/CAMBdemo.html>`_ for a quick
...
You will need gfortran 4.8 or higher installed to compile. Binary files for Windows are also provided, so these are used instead if no
