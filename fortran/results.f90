! Modules used by cmbmain and other routines.
    ...
    integer, parameter :: derived_age = 1, derived_zstar = 2, derived_rstar = 3, derived_thetastar = 4, &
        derived_DAstar = 5, derived_zdrag = 6, derived_rdrag = 7, derived_kD = 8, derived_thetaD = 9, &
        derived_zEQ = 10, derived_keq = 11, derived_thetaEQ = 12, derived_theta_rs_EQ = 13
    ...
    type, extends(TCAMBdata) :: CAMBdata
        ...
        ! C_L/PK not computed; initial power spectrum data, instead get Delta_q_l array
        logical :: OnlyTransfer = .false.
...
10      if (this%CP%omnuh2 < 1.e-7_dl) this%CP%omnuh2 = 0
    ...
    real(dl), intent(in), optional :: in_tol
    ...
    do i = 1, n
        if (i == 1) then
            ...
            else if (z(i) < z(i - 1) + 1e-6_dl) then
                arr(i) = 0
            else
    ...
    if (CP%Do21cm .and. CP%transfer_21cm_cl) then
        do RW_i = 1, CP%Transfer%PK_num_redshifts
            ...
            if (FeedbackLevel > 0) then
                outstr = 'z= ' // trim(RealToStr(real(CP%Transfer%PK_redshifts(RW_i)))) // ': tau_21cm = ' // &
                    trim(RealToStr(real(tau_eps), 5)) // '; T_b = ' // trim(RealToStr(real(Tb21cm), 6)) // 'mK'
                write(*, *) trim(outstr)
            end if
        end do
    end if
    ...
    do j = jstart, TimeSteps%npoints
        ...
        do i = 1, State%num_redshiftwindows
            associate(RedWin => State%Redshift_W(i))
                ...
                else if (RedWin%kind == window_counts) then
    ...
    do i = 1, State%num_redshiftwindows
        associate(RedWin => State%Redshift_W(i))
            ...
            call cubic_spline_integral_array(TimeSteps%points(jstart:), int_tmp(jstart:, i), tmp, tmp2(jstart:), &
                ninterp)
            ...
            if (RedWin%kind == window_21cm) then
                ...
                if (State%CP%SourceTerms%counts_evolve) then
                    ...
                    do ix = jstart, TimeSteps%npoints
                        ...
                        else if (RedWin%comoving_density_ev(ix) /= 0._dl) then
    ...
    if (i < this%nthermo) then
        ...
        this%step_redshift(j2) = this%redshift_time(i) + &
            d*(this%dredshift_time(i) + &
            d*(3._dl*(this%redshift_time(i + 1) - this%redshift_time(i)) - 2._dl*this%dredshift_time(i) - &
            this%dredshift_time(i + 1) + &
            d*(this%dredshift_time(i) + this%dredshift_time(i + 1) + &
            2._dl*(this%redshift_time(i) - this%redshift_time(i + 1)))))

        this%rhos_fac(j2) = this%arhos_fac(i) + &
            d*(this%darhos_fac(i) + d* &
            (3._dl*(this%arhos_fac(i + 1) - this%arhos_fac(i)) - 2._dl*this%darhos_fac(i) - this%darhos_fac(i + 1) + &
            d*(this%darhos_fac(i) + this%darhos_fac(i + 1) + 2._dl*(this%arhos_fac(i) - this%arhos_fac(i + 1)))))
        ...
        do RW_i = 1, State%num_redshiftwindows
            if (State%Redshift_W(RW_i)%has_lensing_window) then
                associate(w => State%Redshift_W(RW_i), C => RW(RW_i))
                    ...
                    w%win_lens(j2) = C%awin_lens(i) + &
                        d*(C%dawin_lens(i) + d* &
                        (3._dl*(C%awin_lens(i + 1) - C%awin_lens(i)) - 2._dl*C%dawin_lens(i) - C%dawin_lens(i + 1) + &
                        d*(C%dawin_lens(i) + C%dawin_lens(i + 1) + 2._dl*(C%awin_lens(i) - C%awin_lens(i + 1)))))
                end associate
            else
    ...
    nextra = (4/minR - State%CP%Transfer%kmax)/h/ &
        (MTrans%TransferData(Transfer_kh, MTrans%num_q_trans, 1) - &
        MTrans%TransferData(Transfer_kh, MTrans%num_q_trans - 1, 1))
