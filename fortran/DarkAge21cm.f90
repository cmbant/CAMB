    ...
    integer, dimension(n_table), parameter :: Temps = [1, 2, 4, 6, 8, 10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, &
    ...
    if (.not. allocated(logRates)) then
        ...
        call cubic_spline_second_derivs(logTemps, logRates, n_table, ddlogRates)
    end if
    ...
    if (deriv) then
        ...
        logRate = &
            a0*logRates(xlo) + b0*logRates(xhi) + ((a0**3 - a0)*ddlogRates(xlo) + (b0**3 - b0)*ddlogRates(xhi))*ho**2/6
