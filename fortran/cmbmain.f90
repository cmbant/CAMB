            ...
            do ell = ThisCT%Limber_l_min(s_ix), ThisCT%ls%nl
                ...
                do n = n1, n2
                    ...
                    if (k <= qmax) then
                        ...
                        LimbRec%Source(n) = sqrt(chi*State%TimeSteps%dpoints(n))* &
                            (a0*ScaledSrc(klo, s_ix, n) + b0*ScaledSrc(khi, s_ix, n) + &
                            (a03*ddScaledSrc(klo, s_ix, n) + b03*ddScaledSrc(khi, s_ix, n))*ho2o6)
                        if (s_ix_lens > 0) then
    ...
    else if (CP%WantTransfer .and. CP%Transfer%k_per_logint /= 0) then
...
101         if (CP%WantTransfer .and. itf <= State%num_transfer_redshifts) then
    ...
    do ik = 1, ThisSources%Evolve_q%npoints
        ...
        else if (ThisSources%Evolve_q%points(ik)/(CP%H0/100) > CP%NonLinearModel%Min_kh_nonlinear) then
    ...
    if (CP%WantScalars) then ! Do Scalars
        ...
        if (ThisSources%SourceNum == 3 .and. (.not. DoInt .or. UseLimber(l))) then
            ...
            if (xf < State%TimeSteps%Highest .and. xf > State%TimeSteps%Lowest) then
                ...
                xf = (xf - State%TimeSteps%points(nbot))/ &
                    (State%TimeSteps%points(nbot + 1) - State%TimeSteps%points(nbot))
